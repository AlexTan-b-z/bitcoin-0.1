...
uint256 SignatureHash(CScript scriptCode, const CTransaction& txTo, unsigned int nIn, int nHashType)
{
    ...
    txTmp.vin[nIn].scriptSig = scriptCode; // 把锁定脚本赋给txTmp的解锁脚本
...
// 判断锁定脚本中的公钥在mapKeys是否存在对应的私钥，如果存在说明此交易是节点本身自己的交易
...
bool ExtractPubKey(const CScript& scriptPubKey, bool fMineOnly, vector<unsigned char>& vchPubKeyRet)
{
    ...
    vector<pair<opcodetype, valtype> > vSolution;
    if (!Solver(scriptPubKey, vSolution)) //从锁定脚本中提取信息（操作和公钥或则公钥哈希）
        return false;
...
bool SignSignature(const CTransaction& txFrom, CTransaction& txTo, unsigned int nIn, int nHashType, CScript scriptPrereq)
{
    ...
    const CTxOut& txout = txFrom.vout[txin.prevout.n]; // 通过prevout找到CTxOut

    // Leave out the signature from the hash, since a signature can't sign itself.
    // The checksig op will also drop the signatures from its hash.
    uint256 hash = SignatureHash(scriptPrereq + txout.scriptPubKey, txTo, nIn, nHashType);

    if (!Solver(txout.scriptPubKey, hash, nHashType, txin.scriptSig)) // 对上一行得到的hash执行签名操作，返回给txin.scriptSig
        return false;
