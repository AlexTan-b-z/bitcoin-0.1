...
void CWalletTx::AddSupportingTransactions(CTxDB& txdb)
{
    ...
    if (SetMerkleBranch() < COPY_DEPTH)
    {
        ...
        CRITICAL_BLOCK(cs_mapWallet)
        {
            ...
            set<uint256> setAlreadyDone; // 同样存放交易输入的hash，代表已经进行过以下操作了
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast)
{
    ...
    // 控制目标难度调整的跨度不能太大也不能太小
	if (nActualTimespan < nTargetTimespan/4)
        nActualTimespan = nTargetTimespan/4;
    if (nActualTimespan > nTargetTimespan*4)
        nActualTimespan = nTargetTimespan*4;

	// 重新计算难度：当前区块对应的前一个区块对应的目标难度 * 实际2016区块对应的创建时间间隔 / 目标时间跨度14天
    // Retarget
    CBigNum bnNew;
    ...
    else if (strCommand == "getdata")
    {
        ...
        foreach(const CInv& inv, vInv)
        {
            ...
            else if (inv.IsKnownType())
            {
                ...
                {
                    map<CInv, CDataStream>::iterator mi = mapRelay.find(inv); // 需要转播的内容
                    if (mi != mapRelay.end())
...
bool SelectCoins(int64 nTargetValue, set<CWalletTx*>& setCoinsRet) // 选出符合这个交易输出的所有交易输入
{  // nTargetValue是转出的钱的数量，setCoinRet返回的交易输入的集合
    setCoinsRet.clear();
    ...
    CRITICAL_BLOCK(cs_main)
    {
        ...
        CRITICAL_BLOCK(cs_mapWallet)
        {
            ...
            loop
            {
                ...
                if (nValue < 0)
                    return false;
                int64 nValueOut = nValue; // 转给别人的钱
                nValue += nFee; // 转给别人的钱加上手续费

                // Choose coins to use
                set<CWalletTx*> setCoins;
                if (!SelectCoins(nValue, setCoins)) // 选择交易输入
                    return false;
                int64 nValueIn = 0;
                foreach(CWalletTx* pcoin, setCoins)
                    nValueIn += pcoin->GetCredit(); // 获取交易输入的总额
                ...
                if (nValueIn > nValue)
                {
                    ...
                    foreach(const CTxOut& txout, txFirst.vout)
                        if (txout.IsMine()) // 判断交易输出是不是输出给自己的（即是不是转给自己的钱）
                            if (ExtractPubKey(txout.scriptPubKey, true, vchPubKey)) // 从锁定脚本中提取公钥
                                break;
                    if (vchPubKey.empty())
                        return false;
                ...
                // Fill vin
                // 填充交易输入里的prevout（上一笔交易输出），并把交易输入放到当前交易里去
                foreach(CWalletTx* pcoin, setCoins)
...
bool SendMoney(CScript scriptPubKey, int64 nValue, CWalletTx& wtxNew)
{
    ...
    {
        int64 nFeeRequired;
		// 新建交易，返回wtxNew(交易),nFeedRequired(交易需要的手续费)
        ...
        // Broadcast
        if (!wtxNew.AcceptTransaction()) // 把wtxNew加到mapTransaction去
        {
