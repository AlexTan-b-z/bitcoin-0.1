...
static const int COINBASE_MATURITY = 100;// 创币交易的成熟度（创币交易要经过100个区块后才能被花费）
// 工作量证明的难度
...
{
public:
    uint256 hash; // 交易对应的hash，这个对应的是CTransaction的hash，而不是CTxOut
...
public:
	...
	// 判断交易的输出是否是节点自己本身的交易，通过锁定脚本里的公钥哈希比对判断
    bool IsMine() const
    {
        return ::IsMine(scriptPubKey);
    }

	// 如果当前交易是转给自己的，就返回数量（即属于自己的余额的一部分）
